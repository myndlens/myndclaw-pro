...
/**
 * MyndLens SB696 (Addenda 473/474) — the zero-token empty "stop" cure, ported
 * from upstream openclaw class #71880/#93073 (scaffold PR #63688).
 *
 * When a provider (live kills: gemini-2.5-pro via google-vertex) returns
 * stopReason "stop" with zero output tokens and no visible content, the run
 * loop retries ONCE, then surfaces a LOUD error — never a silent completion
 * ("No reply from agent." -> completed was the persistent empty-final-frame
 * class).
 */
describe("empty response retry in run loop", () => {
  ...
  it("retries on empty response and succeeds on subsequent attempt", async () => {
    ...
    expect(mockedLog.warn).toHaveBeenCalledWith(expect.stringContaining("[empty-stop-retry]"));
    // 不应该是错误结果
    expect(result.meta.error).toBeUndefined();
  });

  it("retries once and surfaces a loud error when both attempts return empty", async () => {
    // 连续4次返回空响应（1次初始 + 3次重试）
    mockedRunEmbeddedAttempt
      .mockResolvedValueOnce(makeEmptyResponseAttempt())
      .mockResolvedValueOnce(makeEmptyResponseAttempt());
    ...
    const warnCalls = mockedLog.warn.mock.calls.filter(
      (args: unknown[]) => typeof args[0] === "string" && args[0].includes("[empty-stop-retry]"),
    );
    expect(warnCalls).toHaveLength(1);
    // 应有1条 error 日志（重试耗尽）
    expect(mockedLog.error).toHaveBeenCalledWith(expect.stringContaining("[empty-stop-exhausted]"));
    // 返回结果包含用户可见的错误提示
  ...
  it("resets empty response retry counter after a successful response", async () => {
    ...
    expect(mockedLog.error).not.toHaveBeenCalledWith(
      expect.stringContaining("[empty-stop-exhausted]"),
    );
  });
});
