...
Risk: The upstream network helper silently weakened HTTPS checks (CERT_NONE) if secure fetching failed. <br>
Mitigation: FIXED in this MyndLens adaptation (SB612) — the insecure TLS fallback is REMOVED; TLS verification is always on and a verification failure fails loud (Doctrine 1). <br>
Risk: The upstream PDF dependency note used a system-wide pip flag (`--break-system-packages`). <br>
Mitigation: FIXED in this adaptation — the flag is removed; reportlab (needed only for the optional `--pdf` path) must be added to the container image build declaratively. JSON/table output needs no extra packages. <br>
