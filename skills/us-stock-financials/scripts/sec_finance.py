...
def _load_ticker_map() -> dict:
    """SEC's authoritative ticker->CIK map. Cached per process."""
...
def cik_from_name(company_name: str) -> Optional[dict]:
    ...
    # Ticker resolution via SEC's authoritative map BEFORE the fuzzy name search.
    # The pipeline passes tickers (e.g. "TVRD:NASDAQ"); strip any exchange suffix.
    ticker_key = key.split(":")[0].strip().upper()
    if ticker_key:
        try:
            tmap = _load_ticker_map()
            hit = tmap.get(ticker_key)
            if hit and hit.get("cik"):
                return {"name": hit["name"], "cik": hit["cik"], "ticker": hit["ticker"]}
        except Exception:
...
def main():
    ...
    try:
        ...
        if args.search:
            ...
            # Progress -> stderr so `--output json` stdout is pure, parseable JSON.
            print(f"\n✅ Found: {company_info['name']}", file=sys.stderr)
            print(f"   CIK:      {company_info['cik']}", file=sys.stderr)
            print(f"   Ticker:   {company_info.get('ticker', 'N/A')}", file=sys.stderr)
            print(f"   Exchange: {company_info.get('exchange', 'N/A')}", file=sys.stderr)
        ...
        if args.pdf:
            ...
            print(f"✅ PDF report generated: {pdf_path}", file=sys.stderr)
